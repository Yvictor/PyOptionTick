...
def search():
    ...
    groups = ["TXO_{}".format(p.stem.strip('exec_')) for p in dataset_h5_path.rglob('*h5')]
    h5p = None
    rt_list = [{"symbol": p,
                "full_name": p,
                "description": "Option",
    ...
    if query[:10] in groups:
        h5p = dataset_h5_path.joinpath("{}.h5".format(query[:10].replace('TXO', 'exec')))
    elif query[:12] in groups:
        h5p = dataset_h5_path.joinpath("{}.h5".format(query[:12].replace('TXO', 'exec')))
    if h5p:
        with h5py.File(str(h5p), mode='r') as h5f:
            symbols = [("TXO_{}_{}".format(h5p.stem.strip('exec_'), k), h5f[k]['table'].size) for k in h5f.keys()]
        rt_list = [{"symbol": p,
                "full_name": p,
...
def history():
    ...
    resampler = df_tick_selected.resample(resolution)
    df_resample = resampler.agg({'成交價格': ['first', 'max', 'min', 'last'], "成交數量(B or S)": 'sum'})
    df_resample.columns = df_resample.columns.droplevel()
    df_resample.rename(columns={'first': 'open', 'max': 'high', 'min': 'low', 'last': 'close', 'sum': 'vol'}, inplace=True)
